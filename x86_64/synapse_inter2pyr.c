...
#define bDA _p[38]
#define wDA _p[39]
#define tau1_xtra _p[40]
#define tau2_xtra _p[41]
#define e_xtra _p[42]
#define gbar_gabaa_xtra _p[43]
#define i_gabab _p[44]
#define g_gabab _p[45]
#define on_gabab _p[46]
#define W_gabab _p[47]
#define rr_gabab _p[48]
#define i_gabaa _p[49]
#define g_gabaa _p[50]
#define on_gabaa _p[51]
#define W_gabaa _p[52]
#define ICa _p[53]
#define iCatotal _p[54]
#define Wmax _p[55]
#define Wmin _p[56]
#define maxChange _p[57]
#define normW _p[58]
#define scaleW _p[59]
#define pregid _p[60]
#define postgid _p[61]
#define F _p[62]
#define D1 _p[63]
#define D2 _p[64]
#define i_xtra _p[65]
#define g_xtra _p[66]
#define r_gabab _p[67]
#define s_gabab _p[68]
#define r_gabaa _p[69]
#define Capoolcon _p[70]
#define A _p[71]
#define B _p[72]
#define t0 _p[73]
#define Afactor _p[74]
#define dW_gabaa _p[75]
#define tsyn _p[76]
#define fa _p[77]
#define factor_xtra _p[78]
#define Dr_gabab _p[79]
#define Ds_gabab _p[80]
#define Dr_gabaa _p[81]
#define DCapoolcon _p[82]
#define DA _p[83]
#define DB _p[84]
#define v _p[85]
#define _g _p[86]
...
#define gbar_Ca gbar_Ca_inter2pyr
 ...
 "tauD1", 1e-09, 1e+09,
 "tauF", 1e-09, 1e+09,
 "tau2_xtra", 1e-09, 1e+09,
 "tau1_xtra", 1e-09, 1e+09,
 0,0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 88, 3);
...
/*CVODE*/
 ...
 DA = DA  / (1. - dt*( ( - 1.0 ) / tau1_xtra )) ;
 DB = DB  / (1. - dt*( ( - 1.0 ) / tau2_xtra )) ;
 return 0;
}
 /*END CVODE*/
 static int release (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   ...
   ICa = P0b * g_gabab * ( v - ECa ) + P0a * gbar_Ca * VDCCm ( _threadargscomma_ v ) * ( v - ECa ) ;
    ...
    A = A + (1. - exp(dt*(( - 1.0 ) / tau1_xtra)))*(- ( 0.0 ) / ( ( - 1.0 ) / tau1_xtra ) - A) ;
    B = B + (1. - exp(dt*(( - 1.0 ) / tau2_xtra)))*(- ( 0.0 ) / ( ( - 1.0 ) / tau2_xtra ) - B) ;
   g_xtra = gbar_gabaa_xtra * ( B - A ) ;
   i_xtra = g_xtra * ( v - e_xtra ) ;
   }
  return 0;
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 _tsav = t; {
   ...
   t0 = t ;
    ...
    double __state = A;
    double __primary = (A + _args[0] * factor_xtra) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau1_xtra ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau1_xtra ) - __primary );
    A += __primary;
  } else {
 A = A + _args[0] * factor_xtra ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = B;
    double __primary = (B + _args[0] * factor_xtra) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau2_xtra ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau2_xtra ) - __primary );
    B += __primary;
  } else {
 B = B + _args[0] * factor_xtra ;
     }
 } }
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 ...
 on_gabab = 0.0 ;
   ...
   A = 0.0 ;
   B = 0.0 ;
   _ltp = ( tau1_xtra * tau2_xtra ) / ( tau2_xtra - tau1_xtra ) * log ( tau2_xtra / tau1_xtra ) ;
   factor_xtra = - exp ( - _ltp / tau1_xtra ) + exp ( - _ltp / tau2_xtra ) ;
   factor_xtra = 1.0 / factor_xtra ;
   }
